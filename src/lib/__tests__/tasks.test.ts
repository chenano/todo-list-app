...
=> {
    it('should set up real-time s) => {
      const mockChannel = {
s(),
        subscribe: jest.fn(),
      };
      mockSupabaseClient.ch

      const callback 
      taskService.subscribe);

      expect(mockSupabaseClient.channe
      expeedWith(
        'postgre
        {
          event: '*',
     ...
     ',
      ...
      expect(mockChannel.subscribe).toHd();
    });
  });
    ...
    })led();veBeenCaltoHaubscribe).nnel.sckChat(mo  expec );
    lback
